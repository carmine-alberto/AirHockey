...
bool commandBufferUpdate=false;
...
struct ModelData {
    ...
    void init(BaseProject *bp, SkyBoxModel skyboxModel);
    void cleanup();
...
struct TextureData {
    ...
    void createCubicTextureImage(const char *const FName[6]);
...
class BaseProject {
   ...
   protected:
    ...
    virtual void localResizeInit() = 0;
    ...
    void loadGLTFMesh(const char *FName,std::vector<Vertex> vertices, std::vector<uint32_t> indices) {
        ...
        std::cout << FName << " (GLTF) -> V: " << vertices.size()
                  << ", I: " << indices.size() << "\n";
  ...
  // Lesson 13
    ...
    //   vkBindBufferMemory(device, buffer, bufferMemory, 0);
    // } TODO Check new placement: why not working anymore?
    ...
    void mainLoop() {
        while (!glfwWindowShouldClose(window)) {
            glfwPollEvents();
            if(commandBufferUpdate){
              createCommandBuffers();
              commandBufferUpdate=false;
            }
            drawFrame();
        }
...
void ModelData::loadMesh(const char *FName) {
    const std::string MODEL_PATH = "models/";
     tinyobj::attrib_t attrib;
        std::vector<tinyobj::shape_t> shapes;
        std::vector<tinyobj::material_t> materials;
        ...
        std::cout << FName << " (OBJ) -> V: " << vertices.size()
                  << ", I: " << indices.size() << "\n";
}
...
void ModelData::createVertexBuffer() {
    ...
    std::cout  << " Vertices Size: " << vertices.size()
        ...
        BP->createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                     VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                     vertexBuffer, vertexBufferMemory);
...
void ModelData::createIndexBuffer() {
    ...
    BP->createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                 indexBuffer, indexBufferMemory);
...
void ModelData::init(BaseProject *bp, SkyBoxModel skyboxModel){
    BP=bp;
    loadMesh(skyboxModel.ObjFile);
    createVertexBuffer();
    createIndexBuffer();
}
...
void TextureData::createCubicTextureImage(const char *const FName[6]){
    const std::string TEXTURE_PATH = "textures/";
    int texWidth, texHeight, texChannels;
    stbi_uc *pixels[6];
...
void TextureData::createSkyBoxImageView(){
...
void TextureData::init(BaseProject *bp, SkyBoxModel skyboxModel){
    BP=bp;
    createCubicTextureImage(skyboxModel.TextureFile);
